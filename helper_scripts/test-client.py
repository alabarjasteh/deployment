...
#port should match the cart service
cart_url_base="http://127.0.0.1:3000"
cart_add="/api/basket"
cart_modify="/api/basket"
cart_get_user_cart="/api/basket"

def try_add_to_cart(token,p,url=""):
    if url =="":
        url=cart_url_base+cart_add+"/"+str(p)
...
def try_modify_cart(token,p,url=""):
    if url =="":
        url=cart_url_base+cart_modify
    
    headers = {'Content-Type': 'application/json',
    'Authorization': 'Bearer ' + token}
    response = requests.request("PUT", url, headers=headers, params=p)
    return json.loads(response.text), response.status_code


def try_get_cart(token,url=""):
    if url =="":
        url=cart_url_base+cart_get_user_cart
...
def cart_service_test():
    products={}
    #login as admin make random user
    tokens, code = signin(admin_email, admin_password)
    
    random_user_email = "test" + str(random.randint(0, 100)) + "@test.com"
    random_user_pass = "testtest"
    if code != 200:
        print("Step[create user with correct data] failed")
    else:
        _, code = create_user(
            tokens["access_token"], "test", "test", random_user_email,random_user_pass , 1)
        if code != 200:
            print("Step[create user with correct data] failed")
        else:
            print("Step[create user with correct data] succeeded")

    #refresh the previous admin login and make products
    tokens, code = refresh(tokens["access_token"], tokens["refresh_token"])
    if code != 200:
        print("Step[creating products] failed")
    else:
        _, code = create_random_products(tokens["access_token"])
        if code != 200:
            print("Step[creating products] failed")
        else:
            print("Step[creating products] succeeded")

    #refresh the user admin and get all products
    tokens, code = refresh(tokens["access_token"], tokens["refresh_token"])
    if code != 200:
        print("Step[GetProducts] failed")
    else:
        products,code = get_all_products(tokens["access_token"])
        if code != 200:
            print("Step[getting products] failed")
        else:
            print("Step[getting products] succeeded")
    #sign out admin
    tokens, code = refresh(tokens["access_token"], tokens["refresh_token"])
    if code != 200:
        print("Step[signout from admin with correct data] failed")
    else:
        _, code = signout(tokens["access_token"], tokens["refresh_token"])
        if code != 200:
            print("Step[signout from admin with correct data] failed")
        else:
            print("Step[signout from admin with correct data] succeeded")
    #sign in as random user
    tokens, code = signin(random_user_email, random_user_pass)
    if code != 200:
        print("Step[signin with correct data] failed")
    else:
        print("Step[signin with correct data] succeeded")
    #test add cart
    # query ={"id":products[0]['id']}
    # _,code=try_add_to_cart(tokens["access_token"],query["id"])
    # if(code != 200):
    #     print("add cart failed")
    # # test modify
    # tokens, code = refresh(tokens["access_token"], tokens["refresh_token"])
    # if(code != 200):
    #     print("modify refresh token failed")
    # else:
    #     query={"op":"add","id":products[0]['id'],"amount":3}
    #     _,code= try_modify_cart(tokens["access_token"],query)
    #     if code != 200:
    #         print("modify failed")
    #get cart
    # tokens, code = refresh(tokens["access_token"], tokens["refresh_token"])
    # if(code != 200):
    #     print("get cart refresh token failed")
    # else:
    data,code=try_get_cart(tokens["access_token"])
    if code != 200:
